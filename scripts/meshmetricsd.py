...
class MeshtasticTelemetryDaemon:
    ...
    def format_prometheus_output(self, node_id: str, telemetry_data: Dict, device_info: Dict) -> List[str]:
        ...
        for key, value in telemetry_data.items():
            metric_name = f"meshtastic_{key.replace(' ', '_')}" #Using for now to not break existing dashboards
#             metric_name = f"meshtastic_{key.lower().replace(' ', '_')}"  #To use later so that changes upstream dont affect us if case changes.
    ...
    def write_output(self, all_output_lines: List[str], push_to_prometheus: bool = True):
        ...
        if self.prometheus_exporter.push_url and push_to_prometheus:
    ...
    def write_individual_node_output(self, node_id: str, output_lines: List[str], device_info: Dict):
        """Write output for a single node and push to Prometheus immediately"""
        output_dir = self.config.get('output', 'directory')
        individual = self.config.getboolean('output', 'individual_files')
        
        # Write individual file if enabled
        if output_dir and individual:
            self.write_output(output_lines, push_to_prometheus=False)
        
        # Always push to Prometheus immediately for individual nodes
        if self.prometheus_exporter.push_url:
            if self.prometheus_exporter.push_metrics(output_lines):
                self.stats['push_successful'] += 1
                self.logger.debug(f"Successfully pushed metrics for node {node_id} to Prometheus gateway")
            else:
                self.stats['push_failed'] += 1
    ...
    def poll_devices(self, devices: List[Dict]):
        ...
        for device in devices:
            ...
            # Handle individual node output (files + Prometheus push)
            self.write_individual_node_output(node_id, output_lines, device)
            
            # Collect for single file output if not using individual files
        ...
        # Write single file output after polling is complete (if not using individual files)
        if all_output_lines and not individual_files:
            self.write_output(all_output_lines, push_to_prometheus=False)
...
def main():
    parser = argparse.ArgumentParser(
        description=f'Meshtastic Repeater Telemetry Daemon {VERSION}, python version: {python_version()}',
